...
const PARK_ORIENTATION = {
  'Guardians':75,'Tigers':225,'Pirates':115,'Cardinals':215,'Mets':180,
  'Nationals':135,'Braves':35,'Blue Jays':10,'Yankees':205,'Athletics':55,
  'Angels':235,'Rockies':195,'Giants':105,'Dodgers':50,'Padres':275,
  'Royals':5,'Red Sox':60,'Twins':35,'Orioles':25,'White Sox':140,
  'Brewers':200,'Cubs':45,'Reds':155,'Phillies':215,
};
...
async function fetchAllWeather(gameDate, gameTeams = [], venueCoords = {}, gameTimes = {}) {
  const results = {};

  // Build coordinate map — live venue coords take priority over hardcoded
  const coordMap = {};
  for (const [team, info] of Object.entries(PARK_COORDS)) {
    coordMap[team] = info;
  }
  // Override with live venue coordinates from MLB schedule API
  for (const [team, v] of Object.entries(venueCoords)) {
    if (v.lat && v.lon) {
      coordMap[team] = {
        lat:  v.lat,
        lon:  v.lon,
        dome: v.roof === 'dome' || v.roof === 'indoor',
      };
    }
  }

  // Dome parks — always static
  for (const [team, info] of Object.entries(coordMap)) {
    if (info.dome && (!gameTeams.length || gameTeams.includes(team))) {
      results[team] = { t:72, h:50, w:0, d:999, c:'—', l:'dome', s:'Dome', dome:true };
    }
  }

  const outdoor = Object.entries(coordMap)
  ...
  try {
    ...
    responses.forEach((data, idx) => {
      const [team, , gameTime] = outdoor[idx];
      const actualGameTime = gameTimes[team] || gameTime;
      ...
      // Try actual game time first
      if (actualGameTime) {
        // gameTime is like "7:05 PM ET" — convert to local hour
        const match = actualGameTime.match(/(\d+):(\d+)\s*(AM|PM)/i);
        if (match) {
          let hour = parseInt(match[1]);
          const isPM = match[3].toUpperCase() === 'PM';
          if (isPM && hour !== 12) hour += 12;
          if (!isPM && hour === 12) hour = 0;
          // Try ET offset — most games in ET
          const padded = String(hour).padStart(2,'0');
          hi = hourly.time.findIndex(x => x === `${datePrefix}T${padded}:00`);
          // Also try +1 hour (local time may differ)
          if (hi < 0) {
            const padded2 = String((hour+1)%24).padStart(2,'0');
            hi = hourly.time.findIndex(x => x === `${datePrefix}T${padded2}:00`);
          }
        }
      }

      // Fallback to typical game time range
      if (hi < 0) {
        const targets = ['T19:00','T18:00','T20:00','T17:00','T21:00','T16:00','T13:00','T14:00'];
