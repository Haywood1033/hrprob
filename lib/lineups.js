...
async function fetchProbablePitchers(dateStr) {
  try {
    ...
    return games.map(g => ({
      ...
      // Live venue data — overrides hardcoded park lookup
      venue: {
        id:       g.venue?.id,
        name:     g.venue?.name,
        lat:      g.venue?.location?.defaultCoordinates?.latitude,
        lon:      g.venue?.location?.defaultCoordinates?.longitude,
        city:     g.venue?.location?.city,
        state:    g.venue?.location?.stateAbbrev,
        roof:     g.venue?.fieldInfo?.roofType?.toLowerCase().includes('open') ? 'open'
                : g.venue?.fieldInfo?.roofType?.toLowerCase().includes('retract') ? 'retractable'
                : g.venue?.fieldInfo?.roofType?.toLowerCase().includes('indoor') ? 'dome'
                : null,
      },
