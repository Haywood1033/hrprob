...
async function fetchProbablePitchers(dateStr) {
  try {
    ...
    // Fetch pitcher hands from people endpoint
    const ids = [...new Set(results.flatMap(g => [g.awayPitcherId, g.homePitcherId]).filter(Boolean))];
    if (ids.length) {
      try {
        ...
        if (pr.ok) {
          ...
          const handMap = {};
          for (const p of (pd.people || [])) handMap[p.id] = p.pitchHand?.code || null;
          for (const g of results) {
            if (g.awayPitcherId) g.awayPitcherHand = handMap[g.awayPitcherId] || null;
            if (g.homePitcherId) g.homePitcherHand = handMap[g.homePitcherId] || null;
          }
          console.log(`Pitcher hands: ${Object.values(handMap).filter(Boolean).length}/${ids.length} fetched`);
        }
      } catch(e) { console.warn('Pitcher hand fetch failed:', e.message); }
    }
