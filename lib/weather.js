...
async function fetchAllWeather(gameDate, gameTeams = []) {
  ...
  await Promise.allSettled(outdoor.map(async team => {
    ...
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FNew_York&forecast_days=2`;
    try {
      ...
      const targetStr = `${gameDate}T17:00`;
      let hi = hourly.time.findIndex(t => t === targetStr);
