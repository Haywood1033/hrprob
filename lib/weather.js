...
async function fetchAllWeather(gameDate, gameTeams = []) {
  ...
  // Single multi-location request — Open-Meteo supports comma-separated lat/lon arrays
  const lats = outdoor.map(([, info]) => info.lat).join(',');
  const lons = outdoor.map(([, info]) => info.lon).join(',');
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lats}&longitude=${lons}&hourly=temperature_2m,wind_speed_10m,wind_direction_10m,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FNew_York&forecast_days=3`;
  ...
  try {
    ...
    responses.forEach((data, idx) => {
      ...
      // Find best hour index for game time
      const targets = [`${gameDate}T19:00`, `${gameDate}T18:00`, `${gameDate}T17:00`, `${gameDate}T20:00`, `${gameDate}T21:00`];
      let hi = -1;
