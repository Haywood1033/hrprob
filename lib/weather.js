...
const PARK_COORDS = {
  ...
  'Athletics':    { lat:37.752, lon:-122.201,dome:false },
...
async function fetchAllWeather(gameDate, gameTeams = []) {
  ...
  await Promise.allSettled(outdoor.map(async team => {
    const { lat, lon } = PARK_COORDS[team];
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FNew_York&forecast_days=3`;
    try {
      ...
      // Try game time first (7pm ET), then first hour of game date, then index 12
      const targets = [`${gameDate}T19:00`, `${gameDate}T18:00`, `${gameDate}T17:00`, `${gameDate}T20:00`];
      let hi = -1;
      for (const t of targets) { hi = hourly.time.findIndex(x => x === t); if (hi >= 0) break; }
      if (hi < 0) hi = hourly.time.findIndex(t => t.startsWith(gameDate));
      if (hi < 0) hi = 12;
      const t    = Math.round(hourly.temperature_2m[hi]);
      const h    = Math.round(hourly.relative_humidity_2m[hi]);
      ...
      results[team] = { t, h, w, d: wdir, c: CARDS[Math.round(wdir/22.5)%16], l: windLabel(wdir, team), s: weatherCode(code), dome: false, src: 'live' };
    } catch(e) { console.warn(`Weather ${team}:`, e.message); }
  }));
