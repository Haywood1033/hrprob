...
async function fetchPitcherStatcast(year) {
  const text = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/custom?year=${year}&type=pitcher&filter=&sort=4&sortDir=asc&min=20&selections=xera,whiff_percent,p_hr_per9,barrel_batted_rate,hard_hit_percent,exit_velocity_avg,pa&csv=true`
  );
  return parseCSV(text);
}
...
function buildPitcherStats(pitcherRows) {
  ...
  for (const row of pitcherRows) {
    ...
    const hr9Raw    = getNum(row,'p_hr_per9','hr_per9');
