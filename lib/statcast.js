...
// Fetch pitcher arsenal (pitch mix, whiff%, velo per pitch type)
async function fetchPitcherArsenal(year) {
  const text = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/pitch-arsenal-stats?type=pitcher&pitchType=&year=${year}&team=&min=100&csv=true`
  );
  return parseCSV(text);
}

// Fetch batter performance vs each pitch type
async function fetchBatterVsPitch(year) {
  const text = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/pitch-arsenal-stats?type=batter&pitchType=&year=${year}&team=&min=50&csv=true`
  );
  return parseCSV(text);
}

// Build pitcher arsenal map: { pitcherName: { FF: {pct, whiff, xba, velo}, SL: {...}, ... } }
function buildPitcherArsenal(rows) {
  const arsenal = {};
  for (const row of rows) {
    const name = getName(row);
    if (!name) continue;
    const pitch = row['pitch_type'] || row['pitch_name'] || '';
    if (!pitch) continue;
    const pct   = getNum(row, 'pitch_percent', 'pitch_usage', 'pitch_pct');
    const whiff = getNum(row, 'whiff_percent', 'whiff_pct');
    const xba   = getNum(row, 'xba', 'est_ba', 'batting_avg');
    const velo  = getNum(row, 'release_speed', 'velocity', 'start_speed');
    const run_value = getNum(row, 'run_value_per100', 'run_value');
    if (pct === null) continue;
    if (!arsenal[name]) arsenal[name] = {};
    arsenal[name][pitch] = { pct: +pct.toFixed(1), whiff, xba, velo, run_value };
  }
  return arsenal;
}

// Build batter vs pitch map: { batterName: { FF: {xwoba, whiff, barrel}, SL: {...}, ... } }
function buildBatterVsPitch(rows) {
  const bvp = {};
  for (const row of rows) {
    const name = getName(row);
    if (!name) continue;
    const pitch = row['pitch_type'] || row['pitch_name'] || '';
    if (!pitch) continue;
    const xwoba = getNum(row, 'xwoba', 'est_woba');
    const whiff = getNum(row, 'whiff_percent', 'whiff_pct');
    const barrel= getNum(row, 'barrel_batted_rate', 'barrel_pct');
    const pa    = getNum(row, 'pa', 'total_pa', 'pitches');
    if (!bvp[name]) bvp[name] = {};
    bvp[name][pitch] = { xwoba, whiff, barrel, pa };
  }
  return bvp;
}
...
async function fetchBatterStatcast(year) {
  const text = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/custom?year=${year}&type=batter&filter=&sort=4&sortDir=desc&min=20&selections=xba,xslg,xwoba,exit_velocity_avg,barrel_batted_rate,hard_hit_percent,strikeout_percent,whiff_percent,sweet_spot_percent,avg_launch_angle,ev50,hr,pa&csv=true`
  );
  return parseCSV(text);
}
...
async function fetchBatterL7(year) {
  // Last 7 days rolling — barrel%, hard hit%, exit velo
  // Baseball Savant rolling leaderboard with date range
  const today = new Date();
  const week  = new Date(today - 7 * 24 * 60 * 60 * 1000);
  const fmt   = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  const text  = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/custom?year=${year}&type=batter&filter=&sort=4&sortDir=desc&min=5&selections=exit_velocity_avg,barrel_batted_rate,hard_hit_percent,pa&startDate=${fmt(week)}&endDate=${fmt(today)}&csv=true`
  );
  return parseCSV(text);
}
...
async function fetchPitcherStatcast(year) {
  const text = await fetchCSV(
    `https://baseballsavant.mlb.com/leaderboard/custom?year=${year}&type=pitcher&filter=&sort=4&sortDir=asc&min=10&selections=xera,whiff_percent,p_hr_per9,p_hr_per_fb,p_fly_ball_percent,strikeout_percent,p_k_percent,barrel_batted_rate,hard_hit_percent,exit_velocity_avg,p_home_run,p_formatted_ip,pa&csv=true`
  );
  return parseCSV(text);
}
...
function buildBatterStats(batterRows, expectedRows) {
  ...
  for (const row of batterRows) {
    ...
    stats[name] = {
      ...
      k_pct:   getNum(row,'strikeout_percent','k_percent','p_k_percent') ?? null,
      bb_pct:  getNum(row,'b_bb_percent','bb_percent','walk_percent') ?? null,
      whiff_pct: getNum(row,'whiff_percent','swing_miss_percent') ?? null,
      ...
      la:             getNum(row,'avg_launch_angle','launch_angle_avg') ?? null,
...
function buildPitcherStats(pitcherRows) {
  ...
  for (const row of pitcherRows) {
    ...
    stats[name] = {
      ...
      hr9:     +hr9.toFixed(2),
      hrBBE:   getNum(row,'p_hr_per_fb','hr_per_fb','home_run_per_fly_ball') ?? null,
...
async function fetchAllStatcast(year = 2026) {
  try {
    const [batterRows, expectedRows, pitcherRows, batterBBRows, pitcherBBRows, pitcherArsenalRows, batterVsPitchRows, batterL7Rows] = await Promise.all([
      ...
      fetchPitcherArsenal(year).catch(e => { console.warn('Pitcher arsenal failed:', e.message); return []; }),
      fetchBatterVsPitch(year).catch(e => { console.warn('Batter vs pitch failed:', e.message); return []; }),
      fetchBatterL7(year).catch(e => { console.warn('Batter L7 failed:', e.message); return []; }),
    ]);

    console.log(`Raw rows: ${batterRows.length} batters ev, ${expectedRows.length} batters exp, ${pitcherRows.length} pitchers, ${batterBBRows.length} batter BB, ${pitcherBBRows.length} pitcher BB, ${pitcherArsenalRows.length} pitcher arsenal, ${batterVsPitchRows.length} batter vs pitch, ${batterL7Rows.length} batter L7`);

    const batters  = buildBatterStats(batterRows, expectedRows);
    const pitchers = buildPitcherStats(pitcherRows);
    const pitcherArsenal = buildPitcherArsenal(pitcherArsenalRows);
    const batterVsPitch  = buildBatterVsPitch(batterVsPitchRows);
    ...
    const arsenalCount  = Object.keys(pitcherArsenal).length;
    const bvpCount      = Object.keys(batterVsPitch).length;

    // Build L7 map — { name: { ev7, barrel7, hh7, pa7 } }
    const batterL7 = {};
    for (const row of batterL7Rows) {
      const name = getName(row);
      if (!name) continue;
      const ev7     = getNum(row, 'exit_velocity_avg', 'launch_speed') ?? null;
      const barrel7 = getNum(row, 'barrel_batted_rate', 'barrel_pct') ?? null;
      const hh7     = getNum(row, 'hard_hit_percent', 'hard_hit_pct') ?? null;
      const pa7     = getNum(row, 'pa') ?? null;
      batterL7[name] = { ev7, barrel7, hh7, pa7 };
      const norm = name.normalize('NFD').replace(/[\u0300-\u036f]/g,'');
      if (norm !== name) batterL7[norm] = batterL7[name];
    }

    console.log(`Built: ${Object.keys(batters).length} batters (${batterWithBB} with pull%), ${Object.keys(pitchers).length} pitchers (${pitcherWithBB} with gb%), ${arsenalCount} pitcher arsenals, ${bvpCount} batter vs pitch, ${Object.keys(batterL7).length} batter L7`);

    return { batters, pitchers, pitcherArsenal, batterVsPitch, batterL7, timestamp: new Date().toISOString(), source: 'baseballsavant.mlb.com', year };
  } catch (err) {
