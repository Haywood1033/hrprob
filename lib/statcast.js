...
async function fetchAllStatcast(year = 2026) {
  try {
    const [batterRows, expectedRows, pitcherRows, batterBBRows, pitcherBBRows, pitcherArsenalRows, batterVsPitchRows] = await Promise.all([
    ...
    console.log(`Raw rows: ${batterRows.length} batters ev, ${expectedRows.length} batters exp, ${pitcherRows.length} pitchers, ${batterBBRows.length} batter BB, ${pitcherBBRows.length} pitcher BB, ${pitcherArsenalRows.length} pitcher arsenal, ${batterVsPitchRows.length} batter vs pitch`);
    ...
    console.log(`Built: ${Object.keys(batters).length} batters (${batterWithBB} with pull%), ${Object.keys(pitchers).length} pitchers (${pitcherWithBB} with gb%), ${arsenalCount} pitcher arsenals, ${bvpCount} batter vs pitch`);

    return { batters, pitchers, pitcherArsenal, batterVsPitch, timestamp: new Date().toISOString(), source: 'baseballsavant.mlb.com', year };
  } catch (err) {
