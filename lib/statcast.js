...
function buildBatterStats(batterRows, expectedRows) {
  ...
  if (expectedRows.length > 0) {
    const cols = Object.keys(expectedRows[0]);
    console.log('Expected stats columns:', cols.join(', '));
    // Find any EV-related columns
    const evCols = cols.filter(k => k.toLowerCase().includes('ev') || k.toLowerCase().includes('exit'));
    console.log('EV-related columns:', evCols.join(', '));
  }
